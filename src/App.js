...
// ── constants ──────────────────────────────────────────────────────────────
const TAX_RATE = 0.25; // NY self-employment ~25% reserve suggestion
...
export default function App() {
  ...
  // persistent state
  const [incomes,   setIncomes]   = useState(() => load("rl_incomes",   []));
  const [purchases, setPurchases] = useState(() => load("rl_purchases", []));
  const [inventory, setInventory] = useState(() => load("rl_inventory", []));
  const [expenses,  setExpenses]  = useState(() => load("rl_expenses",  []));

  useEffect(() => save("rl_incomes",   incomes),   [incomes]);
  useEffect(() => save("rl_purchases", purchases), [purchases]);
  useEffect(() => save("rl_inventory", inventory), [inventory]);
  useEffect(() => save("rl_expenses",  expenses),  [expenses]);
  ...
  const totalPurchase = monthPurchases.reduce((s,p) => s + (p.totalCost||0), 0);
  ...
  const sharedProps = { mk, selYear, selMonth, setSelYear, setSelMonth,
  ...
  return (
    <div className="app">
      ...
      <main className="main">
        ...
        {tab === "purchase"  && <Purchase  {...sharedProps} />}
        {tab === "inventory" && <Inventory {...sharedProps} />}
        {tab === "expenses"  && <Expenses  {...sharedProps} />}
        {tab === "tax"       && <Tax       {...sharedProps} incomes={incomes} expenses={expenses} purchases={purchases} selYear={selYear} />}
      </main>
    </div>
  );
}
...
function Dashboard({ selYear, selMonth, totalIncome, totalPurchase, totalExpense, grossProfit, taxReserve, netAfterTax, monthIncomes, inventory }) {
  ...
  const invValue = inventory.filter(i=>i.status==="In Stock").reduce((s,i)=>s+(i.costPrice||0)*(i.qty||1),0);
  const invCount = inventory.filter(i=>i.status==="In Stock").reduce((s,i)=>s+(i.qty||1),0);
    ...
    <section>
      ...
      <div className="row-2col">
        ...
        <div className="card">
          ...
          {inventory.filter(i=>i.status==="In Stock").slice(0,4).map(i=>(
          ...
          {inventory.filter(i=>i.status==="In Stock").length > 4 &&
            <p className="muted-sm">还有 {inventory.filter(i=>i.status==="In Stock").length-4} 件…</p>}
        </div>
      </div>
    </section>
  );
...
function Income({ mk, monthIncomes, setIncomes }) {
  const blank = { platform:"eBay", amount:"", orderNo:"", note:"", date:"" };
  const [form, setForm] = useState(blank);
  const set = k => e => setForm(f => ({...f,[k]:e.target.value}));

  const add = () => {
    if (!form.amount) return;
    setIncomes(p => [...p, { id:uid(), month:mk, ...form, amount:parseFloat(form.amount), date: form.date||new Date().toLocaleDateString("en-US") }]);
    setForm(blank);
  };

  const del = id => setIncomes(p => p.filter(x=>x.id!==id));
    ...
    <section>
      <div className="card">
        ...
        <div className="form-grid">
          ...
          <label>金额 ($)
            <input type="number" value={form.amount} onChange={set("amount")} placeholder="0.00" min="0" />
          </label>
          <label>订单号 / 参考
            <input value={form.orderNo} onChange={set("orderNo")} placeholder="可选" />
          </label>
          <label>日期
            <input type="date" value={form.date} onChange={set("date")} />
          </label>
          <label className="span2">备注
            <input value={form.note} onChange={set("note")} placeholder="商品描述、备注…" />
          </label>
        </div>
        <button className="btn-primary" onClick={add}>添加收入</button>
      </div>
            ...
            <tbody>
              {monthIncomes.map(i => (
                <tr key={i.id}>
                  ...
                  <td className="mono">{i.orderNo||"—"}</td>
                  <td>{i.note||"—"}</td>
...
function Purchase({ mk, monthPurchases, setPurchases, setInventory }) {
  const blank = { name:"", sku:"", buyPrice:"", shipping:"", other:"", qty:"1", note:"", date:"", image:null };
  const [form, setForm] = useState(blank);
  const set = k => e => setForm(f => ({...f,[k]:e.target.value}));
  const imgRef = useRef();
  ...
  const add = () => {
    ...
    setInventory(inv => [...inv, {
      id: uid(), sku: form.sku, name: form.name,
      costPrice: unitCost, qty: parseInt(form.qty)||1,
      status: "In Stock", note: form.note, image: form.image,
      addedMonth: mk,
    }]);
    setForm(blank);
  };
  ...
  return (
    <section>
      <div className="card">
        ...
        <div className="form-grid">
          <label>商品名称
            <input value={form.name} onChange={set("name")} placeholder="例：Jordan 1 Retro High" />
          </label>
          <label>SKU / 编号
            <input value={form.sku} onChange={set("sku")} placeholder="例：JD1-001" />
          </label>
          <label>收购单价 ($)
            <input type="number" value={form.buyPrice} onChange={set("buyPrice")} placeholder="0.00" min="0"/>
          </label>
          <label>运费 ($)
            <input type="number" value={form.shipping} onChange={set("shipping")} placeholder="0.00" min="0"/>
          </label>
          <label>其他费用 ($)
            <input type="number" value={form.other} onChange={set("other")} placeholder="关税、包装" min="0"/>
          </label>
          <label>数量
            <input type="number" value={form.qty} onChange={set("qty")} placeholder="1" min="1"/>
          </label>
          <label>日期
            <input type="date" value={form.date} onChange={set("date")} />
          </label>
          <label>备注
            <input value={form.note} onChange={set("note")} placeholder="来源、状况…" />
          </label>
        ...
        <button className="btn-primary" onClick={add}>添加收货（自动入库）</button>
      </div>
            ...
            {monthPurchases.map(p => (
              <div key={p.id} className="purchase-item">
                ...
                <div className="purchase-info">
                  ...
                  <div className="purchase-meta">数量 {p.qty} · 单件 {fmt$(p.unitCost)} · <strong>总计 {fmt$(p.totalCost)}</strong></div>
...
function Inventory({ inventory, setInventory }) {
  ...
  const statuses = ["All","In Stock","Listed","Sold","Returned"];
  ...
  const updateStatus = (id, status) => setInventory(inv => inv.map(i => i.id===id ? {...i,status} : i));
  const updateNote   = (id, note)   => setInventory(inv => inv.map(i => i.id===id ? {...i,note}   : i));
  const updateImg    = (id, image)  => setInventory(inv => inv.map(i => i.id===id ? {...i,image}  : i));
  const del          = id           => setInventory(inv => inv.filter(i => i.id!==id));

  const handleImg = e => {
    const file = e.target.files[0];
    if (!file || !imgTarget) return;
    const reader = new FileReader();
    reader.onload = ev => { updateImg(imgTarget, ev.target.result); setImgTarget(null); };
    reader.readAsDataURL(file);
  };
            ...
            {filtered.map(item => (
              <div key={item.id} className="inv-card">
                <div className="inv-img-wrap" onClick={() => { setImgTarget(item.id); imgRef.current.click(); }}>
                  {item.image
                    ? <img src={item.image} alt="" className="inv-img" />
                ...
                <div className="inv-body">
                  ...
                  <div className="inv-item-meta">
                    成本 {fmt$(item.costPrice)} · ×{item.qty||1}
                  </div>
                  <select
                    className={`status-select s-${statusColor[item.status]||"muted"}`}
                    value={item.status}
...
function AddInventoryManual({ setInventory }) {
  const blank = { name:"", sku:"", costPrice:"", qty:"1", status:"In Stock", note:"", image:null };
  const [form, setForm] = useState(blank);
  ...
  const add = () => {
    if(!form.name) return;
    setInventory(inv => [...inv, { id:uid(), ...form, costPrice:parseFloat(form.costPrice)||0, qty:parseInt(form.qty)||1 }]);
    ...
    <div className="card">
      ...
      {open && <>
        <div className="form-grid" style={{marginTop:16}}>
          ...
          <label>成本价 ($) <input type="number" value={form.costPrice} onChange={set("costPrice")} placeholder="0.00" /></label>
        ...
        <button className="btn-primary" onClick={add}>添加到库存</button>
      </>}
    </div>
  );
}
...
function Expenses({ mk, monthExpenses, setExpenses }) {
  ...
  const add = () => {
    ...
    setExpenses(p=>[...p,{id:uid(),month:mk,...form,amount:parseFloat(form.amount),date:form.date||new Date().toLocaleDateString("en-US")}]);
    setForm(blank);
  };
  const del = id => setExpenses(p=>p.filter(x=>x.id!==id));
  ...
  return (
    <section>
      <div className="card">
        ...
        <button className="btn-primary" onClick={add}>添加支出</button>
      </div>
...
function Tax({ selYear, incomes, expenses, purchases }) {
  const yearIncome   = incomes.filter(i=>i.month?.startsWith(selYear)).reduce((s,i)=>s+i.amount,0);
  const yearPurchase = purchases.filter(p=>p.month?.startsWith(selYear)).reduce((s,p)=>s+p.totalCost,0);
  const yearExpense  = expenses.filter(e=>e.month?.startsWith(selYear)).reduce((s,e)=>s+e.amount,0);
  const yearProfit   = yearIncome - yearPurchase - yearExpense;
  const selfEmpTax   = Math.max(0, yearProfit) * 0.153;  // SE tax ~15.3%
  const fedIncomeTax = Math.max(0, yearProfit - selfEmpTax/2) * 0.22; // ~22% bracket estimate
  const nyStateTax   = Math.max(0, yearProfit) * 0.0685; // NY state ~6.85%
  const nycTax       = Math.max(0, yearProfit) * 0.03876; // NYC local
  const total1099    = selfEmpTax + fedIncomeTax + nyStateTax + nycTax;

  const rows = [
    { label:"年度总收入", val: fmt$(yearIncome), note:"eBay + Mercari + 其他" },
    { label:"年度收货成本", val: fmt$(yearPurchase), note:"COGS — 可抵税" },
    { label:"年度运营支出", val: fmt$(yearExpense), note:"Business expenses — 可抵税" },
    { label:"净利润 (应税收入)", val: fmt$(yearProfit), note:"Schedule C 利润" },
    { label:"自雇税 (SE Tax 15.3%)", val: fmt$(selfEmpTax), note:"社保 + 医保" },
    { label:"联邦所得税 (~22%)", val: fmt$(fedIncomeTax), note:"估算，实际因收入总额而异" },
    { label:"NY 州税 (6.85%)", val: fmt$(nyStateTax), note:"New York State" },
    { label:"NYC 市税 (3.876%)", val: fmt$(nycTax), note:"New York City residents" },
  ];

  const qtr = Math.max(0, total1099) / 4;

  return (
    <section>
      <div className="card tax-disclaimer">
        <p>⚠️ 以下为 <strong>{selYear} 年度</strong>税款估算，仅供参考。请咨询专业税务顾问（CPA）获取准确数据。</p>
      </div>
      <div className="card">
        ...
        <table className="tbl">
          ...
          <tfoot>
            <tr className="tax-total">
              <td>预计总税款</td>
              <td className="red mono">{fmt$(total1099)}</td>
              <td></td>
            </tr>
          </tfoot>
        </table>
      ...
      <div className="card">
        <h2 className="card-title">📅 季度预缴税 (Estimated Tax)</h2>
        ...
        <div className="qtr-dates">
          ...
          <div><strong>Q4</strong> Jan 15 (次年)</div>
        </div>
...
function Style() {
  return <style>{`
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    :root{
      --bg:#0a0c0f; --s1:#111418; --s2:#1a1f27; --border:#252b35;
      --text:#e8edf5; --muted:#6b7585; --accent:#f0b429;
      --green:#3fb950; --red:#f85149; --blue:#58a6ff;
      --orange:#e3873a; --teal:#39d353; --yellow:#f0b429;
    }
    ...
    .header-top{display:flex;align-items:center;justify-content:space-between;padding:16px 20px 0;flex-wrap:wrap;gap:10px}
    .logo{font-family:'Syne',sans-serif;font-size:1.2rem;font-weight:800;letter-spacing:2px;color:var(--accent)}
    ...
    .tabs{display:flex;overflow-x:auto;padding:0 16px;gap:2px;-webkit-overflow-scrolling:touch;scrollbar-width:none}
    .tabs::-webkit-scrollbar{display:none}
    .tab{background:transparent;border:none;border-bottom:3px solid transparent;color:var(--muted);cursor:pointer;padding:10px 14px;font-family:'Noto Sans SC',sans-serif;font-size:.85rem;font-weight:500;white-space:nowrap;transition:all .2s}
    ...
    .stat-card .stat-label{font-size:.74rem;opacity:.7;font-weight:500}
    .stat-card .stat-val{font-family:'DM Mono',monospace;font-size:1.25rem;font-weight:500}
    .stat-card .stat-sub{font-size:.7rem;opacity:.5}
    ...
    .plat-amt{font-family:'DM Mono',monospace;font-size:.82rem;color:var(--text);min-width:60px;text-align:right}
    ...
    /* BUTTONS */
    .btn-primary{background:var(--accent);color:#0a0c0f;border:none;border-radius:9px;padding:11px 24px;font-size:.9rem;font-weight:700;font-family:'Noto Sans SC',sans-serif;cursor:pointer;transition:opacity .2s,transform .1s;width:100%}
    ...
    .del{background:transparent;border:none;color:var(--red);cursor:pointer;padding:3px 7px;border-radius:5px;font-size:.82rem;transition:background .2s;opacity:.6}
    ...
    .note-edit input{font-size:.78rem;padding:5px 8px}
